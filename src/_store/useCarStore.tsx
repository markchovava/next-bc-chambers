...
import { carByPriceAction, carByYearAction, carPaginateAction, carSearchAction } from "@/app/admin/_data/actions/CarActions"
...
interface Props {
    ...
    search: string
    isSearching: boolean
    setCarBrandList: (i: BrandInterface[]) => void
...
export const useCarStore = create<Props>((set, get) => ({
    ...
    getPaginatedDatalist: async (url: string) => {
        set({ isLoading: true });
        try {
            const res = await carPaginateAction(url);
            if (res && res.data && res.meta && res.links) {
                set({
                    dataList: res.data,
                    meta: res.meta,
